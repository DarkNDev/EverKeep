package com.darkndev.everkeep.ui.note_recycle

import android.os.Bundle
import android.view.ActionMode
import android.view.Menu
import android.view.MenuInflater
import android.view.MenuItem
import android.view.View
import android.widget.Toast
import androidx.core.view.MenuHost
import androidx.core.view.MenuProvider
import androidx.drawerlayout.widget.DrawerLayout
import androidx.fragment.app.Fragment
import androidx.fragment.app.viewModels
import androidx.lifecycle.Lifecycle
import androidx.lifecycle.lifecycleScope
import androidx.navigation.fragment.findNavController
import androidx.recyclerview.selection.SelectionTracker
import androidx.recyclerview.selection.StorageStrategy
import com.darkndev.everkeep.R
import com.darkndev.everkeep.databinding.FragmentRecycleBinding
import com.darkndev.everkeep.recyclerview.adapters.NoteAdapter
import com.darkndev.everkeep.recyclerview.decoration.ItemOffsetDecoration
import com.darkndev.everkeep.recyclerview.selection.NoteActionMode
import com.darkndev.everkeep.recyclerview.selection.SelectionDetailsLookup
import com.darkndev.everkeep.recyclerview.selection.SelectionKeyProvider
import com.google.android.material.snackbar.Snackbar
import dagger.hilt.android.AndroidEntryPoint
import kotlinx.coroutines.flow.collectLatest
import kotlinx.coroutines.launch

private const val ITEM_SELECT = "com.darkndev.everkeep.ui.note_recycle.ITEM_SELECT"

@AndroidEntryPoint
class NoteRecycleFragment : Fragment(R.layout.fragment_recycle), MenuProvider {

    private var _binding: FragmentRecycleBinding? = null
    private val binding get() = _binding!!

    private val viewModel: NoteRecycleViewModel by viewModels()

    private lateinit var noteAdapter: NoteAdapter

    private lateinit var tracker: SelectionTracker<Long>

    private lateinit var callback: NoteActionMode
    private var actionMode: ActionMode? = null

    private lateinit var drawerLayout: DrawerLayout

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        _binding = FragmentRecycleBinding.bind(view)

        drawerLayout = requireActivity().findViewById(R.id.drawer_layout)

        noteAdapter = NoteAdapter {
            Toast.makeText(view.context, "Restore to open", Toast.LENGTH_SHORT).show()
        }

        callback = NoteActionMode(R.menu.menu_recycled_action_mode, onActionItemClicked = {
            when (it) {
                R.id.action_restore -> {
                    viewModel.restoreNotes(tracker.selection.toList())
                    true
                }

                R.id.action_delete -> {
                    viewModel.deleteNotes(tracker.selection.toList())
                    true
                }

                else -> false
            }
        }, onDestroyActionMode = {
            tracker.clearSelection()
            actionMode = null
        })

        binding.apply {
            val menuHost: MenuHost = requireActivity()

            menuHost.addMenuProvider(
                this@NoteRecycleFragment,
                viewLifecycleOwner,
                Lifecycle.State.RESUMED
            )


            viewModelBinding = viewModel
            lifecycleOwner = viewLifecycleOwner

            recyclerViewArchived.apply {
                addItemDecoration(ItemOffsetDecoration(8))
                adapter = noteAdapter
            }

            tracker = SelectionTracker.Builder(
                ITEM_SELECT,
                binding.recyclerViewArchived,
                SelectionKeyProvider(binding.recyclerViewArchived),
                SelectionDetailsLookup(binding.recyclerViewArchived),
                StorageStrategy.createLongStorage()
            ).build()

            noteAdapter.setSelection(tracker)
        }

        viewModel.items.observe(viewLifecycleOwner) {
            noteAdapter.submitList(it)
        }

        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.noteRecycleEvent.collectLatest { event ->
                when (event) {
                    is NoteRecycleViewModel.NoteRecycleEvent.NavigateToConfirmDialog -> {
                        val action = NoteRecycleFragmentDirections.actionGlobalConfirmDialog(
                            event.requestCode,
                            event.idsList.toLongArray(),
                            event.title,
                            event.message,
                            event.positiveText,
                            event.negativeText
                        )
                        findNavController().navigate(action)
                    }

                    is NoteRecycleViewModel.NoteRecycleEvent.ShowMessage -> {
                        Snackbar.make(view, event.message, Snackbar.LENGTH_SHORT)
                            .setAction("UNDO") {
                                viewModel.restoreNotes(event.idsList, true)
                            }
                            .show()
                    }
                }
            }
        }

        selectionTrackerObserver(view)
    }

    private fun selectionTrackerObserver(view: View) {
        tracker.addObserver(object : SelectionTracker.SelectionObserver<Long>() {
            override fun onItemStateChanged(key: Long, selected: Boolean) {
                if (actionMode == null && tracker.hasSelection()) {
                    actionMode = view.startActionMode(callback)
                    drawerLayout.setDrawerLockMode(DrawerLayout.LOCK_MODE_LOCKED_CLOSED)
                } else if (actionMode != null && tracker.selection.isEmpty) {
                    actionMode?.finish()
                    drawerLayout.setDrawerLockMode(DrawerLayout.LOCK_MODE_UNLOCKED)
                }
                actionMode?.title = "${tracker.selection.size()} Selected"
            }
        })
    }

    override fun onCreateMenu(menu: Menu, menuInflater: MenuInflater) {
        menuInflater.inflate(R.menu.menu_recycle, menu)
    }

    override fun onMenuItemSelected(menuItem: MenuItem) =
        if (menuItem.itemId == R.id.action_delete_all) {
            val idsList = ArrayList<Long>()
            noteAdapter.currentList.forEach {
                idsList.add(it.id)
            }
            if (idsList.isNotEmpty()) viewModel.deleteAllNotes(idsList) else Toast.makeText(
                view?.context,
                "Recycle Bin is Empty",
                Toast.LENGTH_SHORT
            ).show()
            true
        } else {
            false
        }

    override fun onDestroyView() {
        super.onDestroyView()
        _binding = null
    }
}